...
| `database.url` | `""` | Inline PostgreSQL URL for quick installs; required unless `database.existingSecret.name` is set |
| `database.existingSecret.name` | `""` | Existing Secret containing `TROVE_DATABASE_URL`; required unless `database.url` is set |
